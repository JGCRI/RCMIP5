...
```
> worldPlot(co2, time=1:12)
```
**TODO - get structure from Corinne**
...
```
> co2surf <- filterDimensions(co2, Zs=co2$Z[1])
...
The first step, `makeAnnualStat`, was relatively slow (and can be quite slow for large datasets).^[All the `make...Stat` functions have a optional `parallel` parameter that will use the `foreach` package and any available parallelized backend.] Also note that because we didn't supply any grid cell area data to the `makeAnnualStat` function, it computed its own.
In any event, now we look at the resulting data:
