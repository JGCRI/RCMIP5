...
The CMIP5 data files can be *very* large, such that loading (not to mention processing) them can be slow or impossible even on computers with plenty of memory. Some tips on dealing with this:

* Use the `yearRange` parameter of `loadCMIP5()` to process smaller chunks of the data, re-combining only at the end. For the largest CMIP5 data sets, we've found it necessary to process 1-5 years at a time, even on a powerful machine.
