...
No grid areas supplied; using calculating values
Calculating grid cell areas...
Running in serial
Took 0.03 s
```

The second step, `makeAnnualStat`, was relatively slow (and can be *very* slow for large datasets).^[All the `make...Stat` functions have a optional `parallel` parameter that will use the `foreach` package and any available parallelized backend.] Also note that because we didn't supply any grid cell area data to the `makeAnnualStat` function, it computed its own.
Now we look at the resulting data:
