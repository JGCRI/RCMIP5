...
makeAnnualStat <- function(x, verbose=TRUE, parallel=FALSE, FUN=mean, ...) {
    ...
    # Make sure these data haven't already been summarized
    ...
    # Check that the dimentions of the value is has at least lon, lat, time
    # (length 3) or 4 (depth or lev) or 5 (both depth and 5)
    stopifnot(length(dim(x$val)) %in% c(3, 4, 5))
