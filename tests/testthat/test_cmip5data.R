...
test_that("cmip5data generates annual and monthly data", {
    ...
    expect_equal(length(dim(d$lon)), 2)
    expect_equal(length(dim(d$lat)), 2)
    expect_equal(length(d$time), 12)
    expect_equal(d$debug$timeFreqStr, "mon")
    # Data-frame specific tests
    expect_equal(length(unique(d$val$lon)), length(unique(as.numeric(d$lon))))
    expect_equal(length(unique(d$val$lat)), length(unique(as.numeric(d$lat))))
    expect_equal(length(unique(d$val$time)), length(unique(as.numeric(d$time))))
    ...
    expect_equal(length(unique(d$val$lon)), length(unique(as.numeric(d$lon))))
    expect_equal(length(unique(d$val$lat)), length(unique(as.numeric(d$lat))))
    expect_equal(length(unique(d$val$time)), length(unique(d$time)))
