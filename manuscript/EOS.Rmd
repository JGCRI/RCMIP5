...
**Introduction to CMIP5 [ktb]**

Introduction.
the Coupled Model Intercomparison Project Phase 5 (CMIP5).

**Need for CMIP5 summaries?**

Everyone has to summarize CMIP5 data in some way to work with it, reinventing wheel each time. This is nontrivial, and because of the many quirks of the CMIP5 database (despite best intentions of modeling teams, whom we applaud):

* different grids, years
* inconsistencies in netcdf file format and assumptions
* other?


**R, reproducibility/provenance, open science [bbl]**

Here we introduce the *RCMIP5* package--collection of software tools--for working with CMIP5 data. This package aims to provide a reproducible, robust, and high-performance set of functions to (i) explore what data have been downloaded from CMIP5 archives, (ii) identify missing data, (iii) average (or apply other mathematical operations) across experimental ensembles, (iv) produce both temporal and spatial statistical summaries, and (v) produce easy-to-work-with graphical and data summaries. The package also includes pre-summarized global means for a variety of frequent model outputs such as atmospheric CO2 and global mean temperature.
